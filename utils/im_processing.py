...
def process_im_single_crop_for_network_caffe(im_whole, box, w, h, pixel_means):
    '''
    Cropping the image within the box from the full image
    '''
	if box[0] < 1 or box[1] < 1 or box[2] > im_whole.width or box[3] > im_whole.height:
		offset_x1 = max(1-box[0], 0)
		offset_y1 = max(1-box[1], 0)
...
def process_frame_global_spatial_search_for_network_caffe(im_whole, w_box_init, h_box_init, w_box_out, h_box_out, net_spatial_reduce_factor, pixel_means):
	'''
    Args:
        im_whole: Full input image frame
        w_box_init: width of initial bounding box
        h_box_init: height of the initial bounding box
        w_box_out,h_box_out: width and height of the desired output box
        net_spatial_reduce_factor: To create desired resolution maps for first 
            stage of search
        pixel_means: Mean of pixel values to account for correction, hard-coded
            from the training data (probably)       
    '''
